...
def extract_text(pdf_path: Path) -> PdfDocument:
    ...
    try:
        ...
        raise ValueError(
            f"Failed to read page tree for {pdf_path.name} (possible malformed PDF structure): {exc}"
        ) from exc
    ...
    for i, page in enumerate(pdf_pages, start=1):
        try:
            ...
            print(f"[warn] Failed to extract page {i} from {pdf_path.name}: {exc}")
            continue
        clean = text.strip()
        if clean:
    ...
    if page_count == 0:
        raise ValueError(f"No readable pages found in {pdf_path.name}")
...
def create_upload_zip(skill_root: Path, output_dir: Path) -> Path:
    zip_path = output_dir / f"{skill_root.name}.zip"
...
def create_skill(
    ...
    if not docs:
        joined = "; ".join(failures) if failures else "unknown extraction error"
        raise ValueError(f"Failed to extract text from all PDFs. Details: {joined}")
    ...
    if failures:
        (skill_root / "extraction-warnings.txt").write_text("\n".join(failures) + "\n", encoding="utf-8")
    ...
    zip_path = create_upload_zip(skill_root, output_dir)
    return skill_root, zip_path
...
def main() -> None:
    ...
    try:
        skill_path, zip_path = create_skill(
        ...
        msg = str(exc)
        if "IndirectObject" in msg:
            msg = (
                f"{msg}. Hint: this PDF is likely malformed; try re-saving/printing it as a new PDF "
                "or upgrading pypdf (pip install -U pypdf)."
            )
        raise SystemExit(f"[error] {msg}") from exc

    print(f"[ok] Skill created: {skill_path}")
    print(f"[ok] Upload zip created: {zip_path}")
    print("[next] Upload the generated .zip in OpenAI Storage > Skills.")
